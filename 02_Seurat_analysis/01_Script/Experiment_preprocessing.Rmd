...
WORKSPACE <- "/home/nozaism/Workspace/01_These/01_Project/Myc_Pten_Paper/"
...
OUTPUT_PATH <- (paste0(WORKSPACE,"Output/"))
...
print("You are starting the analysis from count matrix blabla ..")
do <- TRUE
}else{
print ("You are starting analysis from Robj blabla ..")
do <- FALSE
}
...
```{r path1_loading, include=FALSE,eval=(do == TRUE )}
# Load path for files
...
```{r Sample1_loading, include=FALSE,eval=(do == TRUE)}
...
```{r HTO1_loading, include=FALSE,eval=(do == TRUE)}
...
```{r HTO distribution,eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
#HTO distribution
par(mfrow=c(1,1))
par(las=2)
par(mar=c(5,15,3,3))
...
```{r, message=FALSE, include=FALSE,eval=(do == TRUE)}
...
hashtag1 <- FindVariableFeatures(hashtag1, do.plot = F, selection.method = "vst", nfeatures = 2000, display.progress = FALSE)
...
Demultiplexing HTO
```{r Demultiplexing, message=FALSE, include=FALSE,eval=(do == TRUE)}
...
```{r, message=FALSE,eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
...
```{r, message=FALSE,echo=FALSE,eval=(do == TRUE)}
DimPlot(hashtag1, group.by = "MULTI_ID",reduction = "tsne")
```

### HTO margin

```{r, fig.width = 8, fig.height = 7, message=FALSE,eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
Tsne<-data.frame(
...
```{r, fig.height = 4, message=FALSE,eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
...
```{r doublet_negative_removal, results='asis',eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
...
```{r processing_1, include=FALSE,eval=(do == TRUE)}
# OBJECT ONE SAVE
...
```{r mito_vs_nfeatures,eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
...
```{r Sample_loading_2, include=FALSE,eval=(do == TRUE)}
# Create Seurat object and apply filtering   
# Read 10X data
...
```{r,eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
par(mfrow=c(1,1))
par(las=2)
par(mar=c(5,15,3,3))
...
```{r cell_select_2, results='asis',eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
...
## Adding ADT
```{r load_adt,  message=FALSE,eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
# Load in the UMI matrix
...
ADT list :
```{r , results='asis',eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
print (rownames(adt))
```

```{r Significant_PC, include=FALSE,eval=(do == TRUE)}
...
```{r mito_vs_nfeatures2,eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
...
## UMAP
```{r UMAP_HTO_seurat_2,eval=(do == TRUE ), echo=if (do) TRUE else FALSE}
...
```{r Samples_loading, include=FALSE}
...
```{r Significant_PC_merge, include=FALSE}
# Traitement de l'objet
